...
const policy = manifest.battleSemanticIconPolicy;
assert(policy, 'BATTLE_SEMANTIC_ICON_POLICY_MISSING');

const token = String(process.env.DISCORD_TOKEN || '').trim();
assert(token, 'DISCORD_TOKEN_MISSING');
const semanticIds = policy.semantics.map((entry) => entry.id);
assert(semanticIds.length === 12 && new Set(semanticIds).size === semanticIds.length,
    'BATTLE_SEMANTIC_MATRIX_INVALID');
...
try {
    ...
    for (const semanticId of semanticIds) {
        ...
        assert(/^[a-z0-9_]{2,32}$/i.test(name), `BATTLE_SEMANTIC_EMOJI_NAME_INVALID:${name}`);
        ...
        assert(sourcePath.startsWith(`${path.resolve(ASSET_ROOT)}${path.sep}`),
            `BATTLE_SEMANTIC_SOURCE_PATH_INVALID:${semanticId}`);
        assert(fs.existsSync(sourcePath), `BATTLE_SEMANTIC_SOURCE_MISSING:${semanticId}`);
    ...
    assert(mappingPath.startsWith(`${path.resolve(ASSET_ROOT)}${path.sep}`),
        'BATTLE_SEMANTIC_MAPPING_PATH_INVALID');
